...
class DynamicStrokeDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, path_df: pd.DataFrame, transform: Callable) -> None:
        ...
        self._augment_frequencies: npt.NDArray[np.float32] = None
    ...
    def _initialize_tables(self) -> None:
        ...
        for i in range(1, self._max_strokes + 1):
            group = tuple(self.base.get_indices_for_strokes(i, arg)[1]
                          for arg in (None, True, False))
            counts = tuple(len(item) if item is not None else 0
                           for item in group)
            groups.append(group)
...
class BatchedVariableStrokeSampler:
    """Batch sampler class to perform custom random sampling for the
    DynamicStrokeDataSet class. It stratifies an ensemble of batch samplers,
    one representing each stroke count. Each batch sampler samples from a
    shuffled combination of all exact match stroke values, a proportioned
    random sample of clipped characters that contain strokes above the
    sampler's threshold, and a proportioned random sample of padded characters
    that contain strokes below the sampler's threshold. The batch size for all
    is fixed to a specified amount and no remainders are dropped. The object
    combines all of these batches into a single shuffled iterable.

    Args:
        lookup_table (npt.NDArray[np.int_]): An array containing the cumulative
            sum of value counts generated by the DynamicStrokeDataSet class.
        augment_frequencies (npt.NDArray[np.float32]): An array containing
            the scaled proportions of the cumulative sums for the clipped and
            padded augmentations also generated by the DynamicStrokeDataSet
            class.
        batch_size (int): The training minibatch size.
        augment_proportion (float, optional): Sets the proportion of augmented
            clipped and padded observations. Use this to make exact stroke
            matches the dominant class. Defaults to .5.
    """
    def __init__(self,
                 lookup_table: npt.NDArray[np.int_],
                 augment_frequencies: npt.NDArray[np.float32],
                 batch_size: int,
    ...
    def _get_sample_count(self, lookup_table: npt.NDArray[np.int_]) -> int:
    ...
    def _create_samplers(self,
                         ...
                         augment_frequencies: npt.NDArray[np.float32],
                         augment_proportion: float
                         ) -> list[torch.utils.data.BatchSampler]:
    ...
    def _mix_batches(self) -> list[list[int]]:
        batches = []
